function plotPC(N,K,channelstring,design_channelstate,channelstate_range,MaxIterations) % last argument is optional
...
%       3. Performing encoding on each sample 
%       4. SC-decoding
%       5. Repeat at most 'MaxIterations' times, and report the stats
%   ====================================================================
%    Usage: plotPC(N,K,channelstring,design_channelstate,channelstate_range,MaxIterations)
%
%       N             -  Block length
%       K             -  Message length
% 
...
%                             indices are chosen)
% 
%       MaxIterations (optional) - Maximum number of iterations to try, to get a
...
if(errorflag==1)
    ...
    fprintf('\n       channelstring - "AWGN" or "BEC" or "BSC"');
    fprintf('\n       design_channelstate - the channel-state to assume during the PCC run');
...
initPC(N,K,channelstring,design_channelstate,1); %last optional argument '1' says "be silent" with no output at the prompt
...
fprintf('\n * Polar Code designed for %s channel at %s=%f',chstr,paramstr,design_channelstate);
...
for jj=1:length(chrange)
    ...
    for l=1:MCsize
        ...
        uhat = pdecode(y,chstr,chrange(j));
 ...
 titlestr = sprintf('N=%d R=%.2f Polar code performance (design-%s=%.2e)',N,(K/N),paramstr,design_channelstate);
