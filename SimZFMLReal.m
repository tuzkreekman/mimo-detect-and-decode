function SimZFMLReal(n, SNRvec, pamSize, normAnt, normConst)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%            to the SNR values.
%   pamSize - Size of PAM constellation to be transnmitted in each antenna.
%             2-PAM corresponds to {+-1}, 4-PAM corresponds to {+-1, +-3},...
...
%
%   Example: SimZFMLReal(4,[-5:5:25],4,1,1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[tab,normedEnergy, avgEnergy] = genConstalTab(pamSize, normConst);
antennaNorm = 1;
if (normAnt)
...
for (jj = 1 : length(NoiseVarLin))
    ...
    for (kk = 1 : LEN)
        ...
        PC_ML_BER(kk,jj) = length(find(pamDetector(rml./diag(D), pamSize, antennaNorm, normedEnergy) ~= x))/n;
        mmseTmp = length(find(pamDetector(wmmse, pamSize, antennaNorm, normedEnergy) ~= x))/n;
        MMSE_BER(kk,jj) = mmseTmp;
        zfTmp = length(find(pamDetector(wzf, pamSize, antennaNorm, normedEnergy) ~= x))/n;
        ZF_BER(kk,jj) = zfTmp;   
...
xlabel('SNR [dB]');
ylabel('Pe');
title(['n = ' num2str(n) ', normAnt = ' num2str(normAnt) ', pamSize = ' num2str(pamSize) ', normConst = ' num2str(normConst)]);
...
%-----------------------------------------------------------------%
function [tab, normFactor, avgEnergy] = genConstalTab(pamSize, normConst)
% Generate M-PAM constellation and return the average energy of the
% constellation
